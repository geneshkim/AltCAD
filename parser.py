...
#parglare code starts on about line 85

scad_code = r"""//Constants
stuffz = 2/1;
fuzz = 0.2/1+.5;
...
grammar = r"""
    ...
    EXPRESSION : comment | primitive;
    comment : '//';
    primitive : 'cube' | 'sphere' | 'cylinder';
"""

g = parglare.Grammar.from_string(grammar)
parser = parglare.Parser(g)
parse_tree = parser.parse(scad_code)


#Accessing parsed info
comments = parse_tree[0].as_list()
for comment in comments:
    print("Comment:", comment[0])

# Accessing primitives
primitives = parse_tree[1].as_list()
for primitive in primitives:
    print("Primitive:", primitive[0])
